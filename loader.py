...
class DataUtils(object):
    def __init__(self, datafile, outdatafile=None):
        self.datapath = datafile

    def read_data(self, ):
        with codecs.open(self.datapath, 'r', 'utf8') as rf:
            for line in rf.readlines():
                dataiters = line.strip()
                yield dataiters

    def get_labels(self, ):
        dataiters = self.read_data()
        for dataline in dataiters:
            label, sentence = dataline.split('\t')
            yield label

    def get_contents(self, ):
        dataiters = self.read_data()
        for datapair in dataiters:
            label, sentence = datapair.split('\t')
            yield sentence

    @staticmethod
    def data_save(datacontent, outdatafile):
        with codecs.open(outdatafile, 'w', 'utf8') as wf:
...
class TextProcessor(object):
    ...
    def get_train_examples(self, data_dir):
        return self._create_examples(
            self._read_file(os.path.join(data_dir, "train_data_sample.txt")), "train")

    """load dev examples"""
    def get_dev_examples(self, data_dir):
        return self._create_examples(
            self._read_file(os.path.join(data_dir, "dev_data_sample.txt")), "dev")

    """load test examples"""
    def get_test_examples(self, data_dir):
          return self._create_examples(
              self._read_file(os.path.join(data_dir, "dev_data_sample.txt")), "test")

    """set labels"""
    def get_labels(self, data_dir):
        # return ['体育', '财经', '房产', '家居', '教育', '科技', '时尚', '时政', '游戏', '娱乐']
        cmed_file = os.path.join(data_dir, 'whole_data.txt')
        dp = DataUtils(cmed_file)
        labels = set(dp.get_labels())
        # print('labels: ',labels)
        return labels
...
def convert_examples_to_features(examples,label_list, max_seq_length,tokenizer):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if len(tokens_a) > max_seq_length - 2: #算上[CLS] and [SEP] with "- 2"
        ...
        # 对于长度小于max_seq_length的input_ids进行补长处理
        ...
        features = collections.OrderedDict() # 顺序字典
