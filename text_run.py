...
def train():
    ...
    train_examples = TextProcessor().get_train_examples(config.data_dir)  # 返回训练数据对象
    ...
    tf.logging.info("Loading data time cost: %.3f seconds...\n" % (time.time() - start_time))
    ...
    for epoch in range(config.num_epochs):
        ...
        for batch_ids, batch_mask, batch_segment, batch_label in batch_train:
            ...
            _, global_step, train_summaries, train_loss, train_accuracy = session.run([model.optim, model.global_step, merged_summary, model.loss, model.acc],
                                                                                      feed_dict=feed_dict)
...
if __name__ == '__main__':
    ...
    config = TextConfig()
    label_list = TextProcessor().get_labels(config.data_dir)
